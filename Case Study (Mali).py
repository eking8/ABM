...
def save_checkpoint(days_elapsed, state_dict, checkpoint_dir="checkpoints"):
    if not os.path.exists(checkpoint_dir):
        os.makedirs(checkpoint_dir)
        
    slot = "even" if (days_elapsed // 7) % 2 == 0 else "odd"
    filepath = os.path.join(checkpoint_dir, f"checkpoint_{slot}.pkl")
    
    logging.info(f"Saving weekly checkpoint to {filepath}...")
    try:
        with open(filepath, 'wb') as f:
            # Protocol 5 handles deep object graphs (Agents <-> Locations) efficiently
            pickle.dump(state_dict, f, protocol=5)
...
def load_checkpoint(filename, Agent):
    ...
    # Restore class-level data
    Agent.city_probabilities = checkpoint_data['city_probabilities']
    Agent.normalized_prob_values = checkpoint_data['normalized_prob_values']
    Agent.foreign_cities = checkpoint_data['foreign_cities']
    Agent.cumulative_distribution = checkpoint_data['cumulative_distribution']

    return {
        'Agents': checkpoint_data['Agents'],
        'G': checkpoint_data['G'],
        'loc_dic': checkpoint_data['loc_dic'],
        'ongoing_conflicts': checkpoint_data['ongoing_conflicts'],
        'populations': checkpoint_data['populations'],
        'statuses': checkpoint_data['statuses'],
        'countries': checkpoint_data['countries'],
        'total_agents': checkpoint_data['total_agents'],
        'i': checkpoint_data['i'],
        'bpd': checkpoint_data['bpd'],
        'days': checkpoint_data['days'],               
        'current_date': checkpoint_data['current_date'],  
        'locations': checkpoint_data['locations'],
        'birth_rate': checkpoint_data['birth_rate'],
        'frac': checkpoint_data['frac']
    }
...
if checkpoint:
    simulation_state = load_checkpoint(checkpoint, Agent) # Pass Agent class
    Agents = simulation_state['Agents']
    ags = list(Agents.values())
    G = simulation_state['G']
    days = simulation_state['days']
    current_date = simulation_state["current_date"]
    loc_dic = simulation_state["loc_dic"]
    ongoing_conflicts = simulation_state["ongoing_conflicts"]
    populations = simulation_state["populations"]
    statuses = simulation_state["statuses"]
    countries = simulation_state["countries"]
    total_agents = simulation_state["total_agents"]
    i = simulation_state["i"]
    bpd = simulation_state["bpd"]
    birth_rate = simulation_state["birth_rate"]
    frac = simulation_state["frac"]

    locations = simulation_state['locations']

    camps = [loc for loc in locations if getattr(loc, 'iscamp', False)]
    cities = [loc for loc in locations if getattr(loc, 'iscity', False) and getattr(loc, 'top20', True)]
    airports = [loc for loc in locations if getattr(loc, 'hasairport', False)]

    try:
        start_index = dates.get_loc(current_date)
        dates = dates[start_index:]  # Slice the dates series from the correct starting point
    except KeyError:
        print(f"Warning: current_date {current_date} not found in dates series.  Restarting from the beginning.")
...
for current_date in dates: 
    ...
    logging.info(f"Simulating day {days}: {current_date.strftime('%Y-%m-%d')}")
    ...
    for id in Agents:
        ...
        Agents[id].assess_situation_and_move_if_needed(G,loc_dic[Agents[id].location],current_date,roulette=roulette)
    ...
    if days > 0 and days % 7 == 0:
        # Pack everything needed to resume the simulation into a dictionary
        # Inside your simulation step loop when saving:
        if days > 0 and days % 7 == 0:
            simulation_state = {
                'Agents': Agents,
                'G': G,
                ...
                'locations': locations,
                'bpd': bpd,
                'city_probabilities': Agent.city_probabilities,
                'normalized_prob_values': Agent.normalized_prob_values,
                'foreign_cities': Agent.foreign_cities,
                'cumulative_distribution': Agent.cumulative_distribution,
                'birth_rate': birth_rate,
                'frac': frac
            }
            save_checkpoint(days, simulation_state)
